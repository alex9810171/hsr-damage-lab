...
- 支援隊友、星魂、專武、遺器效果資料化。
- 支援儲存配置與分享配置。
- 支援最佳化器屬於後續版本；V1 只做目前配置試算與邊際提升參考。
...
- V1 已讓隊伍 preset 與實際隊友欄位同步；完整自訂隊友 Buff 資料化仍待後續。
- 大黑塔戰技與強化戰技已改用命中分布理解；未來多角色需逐步移到 skill resolver。
...
- 在頁面中顯示乘區係數摘要，例如攻擊區、倍率區、增傷區、暴擊期望、防禦、抗性、易傷、減傷、弱點；暴擊期望需顯示暴率/暴傷，防禦區需顯示目前減防/無視。
...
目前範例詞條分配先固定為攻擊% 5、固定攻擊 0、暴率 15、暴傷 10，讓暴率盡量接近 100%。
...
- 設計可驗證的輸入限制、裝備資料來源與比較目標。
- 明確區分「邊際提升參考」與「配裝搜尋」。
- 在資料核對完成前，不宣稱能計算配裝搜尋結果或產生自動配裝建議。
...
- 後續再拆 `calculator.js` 到 engine / resolver 模組。
- 後續再完整核對遺器與位面飾品效果。
- 後續再資料化隊友 Buff。
- 後續再支援多角色。
- 後續再做真正 optimizer。
...
- Phase 4 已進一步支援指定隊伍欄位：角色、星魂、光錐、Cavern、Planar。
- V1 目前仍不拆 engine、不新增框架、不新增 build step。
- 後續仍需補：可靠技能等級倍率表、那刻夏多弱點戰鬥狀態模型、隊友技能 / 星魂 / 光錐完整資料化、更多遺器與位面效果核對。
- 真正 optimizer 仍維持 Phase 7，不屬於 V1。
